...
### Промпт-инжиниринг
Промпт, передаваемый модель, содержит инструкции о том, как интерпретировать сообщение пользователя. Модель на основе инструкции должна определить, запрашивает ли пользователь информацию о расписании поездов или нет. Разработка правильной инструкции удалась не сразу
...
### Ответ модели
Если модель определяет, что запрос связан с расписанием поездов, она должна сформировать ответ в специальном формате, который включает в себя пункт отправления, пункт назначения, дату и тип запрашиваемой информации (поезд или поезда).
...
Функция parse_response анализирует ответ языковой модели и извлекает необходимую информацию о маршруте и дате, если ответ соответствует формату запроса о расписании поездов.
...
Таким образом, языковая модель YaGPT используется для классификации запросов пользователей на основе заданных инструкций и примеров. Если запрос связан с расписанием поездов, модель должна сформировать ответ в определенном формате, который затем парсится и используется для получения фактического расписания поездов.
