...
Далее ответ ржд был взят как эталонный, ответ RAG как тестовый, оба были векторизированы. Затем между ними было рассчитано косинусное сходство (как между векторами), данный показатель, где **-1** (не похожи) и **1** (практически идентичны) и стал мерой сравнения двух ответов. 

Для наглядности была построена матрица сравнения, где ответы RAG были сравнены по косинусу не только с соответствующим идеальным ответом, но и с другими, в качестве доказательства, что мера сравнения объективна. По итогу по диагонали видно насколько соответствуют ответы RAG идеальным ответам. Матрица:
